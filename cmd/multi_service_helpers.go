...
func getAllAWSRegions(ctx context.Context, cfg aws.Config) ([]string, error) { //nolint:gocritic
...
func determineServicesToProcess(cfg Config) []common.ServiceType { //nolint:gocritic
	if cfg.AllServices {
...
func printPaymentAndTerm(cfg Config) { //nolint:gocritic
...
func generateCSVFilename(isDryRun bool, cfg Config) string { //nolint:gocritic
	if cfg.CSVOutput != "" {
...
func groupRecommendationsByServiceRegion(recommendations []common.Recommendation) map[common.ServiceType]map[string][]common.Recommendation { //nolint:gocritic
	recsByServiceRegion := make(map[common.ServiceType]map[string][]common.Recommendation)
	for _, rec := range recommendations { //nolint:gocritic
...
func populateAccountNames(ctx context.Context, recommendations []common.Recommendation, accountCache *AccountAliasCache) { //nolint:gocritic
...
func createDryRunResult(rec common.Recommendation, region string, index int, cfg Config) common.PurchaseResult { //nolint:gocritic
...
func createCancelledResults(recs []common.Recommendation, region string, cfg Config) []common.PurchaseResult { //nolint:gocritic
...
func executePurchase(ctx context.Context, rec common.Recommendation, region string, index int, serviceClient provider.ServiceClient, cfg Config) common.PurchaseResult { //nolint:gocritic
...
func determineRegionsForService(ctx context.Context, awsCfg aws.Config, recClient provider.RecommendationsClient, service common.ServiceType, configuredRegions []string) ([]string, error) { //nolint:gocritic
...
func fetchEngineVersionData(ctx context.Context, cfg Config) engineVersionData { //nolint:gocritic
...
func queryInstanceVersions(ctx context.Context, cfg Config) map[string][]InstanceEngineVersion { //nolint:gocritic
...
func queryMajorVersions(ctx context.Context, cfg Config) map[string]MajorEngineVersionInfo { //nolint:gocritic
...
func processRegionRecommendations(
	ctx context.Context,
	awsCfg aws.Config, //nolint:gocritic
...
func applyCoverageAndOverrides(recs []common.Recommendation, cfg Config, coverageMap recommendations.PoolCoverageMap, expiringCommitments []common.Commitment) []common.Recommendation { //nolint:gocritic
...
func checkDuplicatesAndApplyLimit(
	...
	if err != nil {
		...
		// filteredRecs unchanged; continue with original recommendations
	} else {
