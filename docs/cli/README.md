...
| `--max-instances` | | `0` (no limit) | Hard cap on the total number of instances purchased across all recommendations. Applied after coverage scaling, once per run across every service and region (not per service or per region). The survivors are the highest-savings recommendations run-wide on both paths; `--input-csv` rows are ranked on their `EstimatedSavings` column, not on file order. A recommendation the cap would truncate below `--min-count` is dropped instead. See [filtering.md](filtering.md). |
