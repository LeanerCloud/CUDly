...
func (s *Scheduler) CollectRecommendations(ctx context.Context) (*CollectResult, error) {
	...
	logging.Infof("Collected %d recommendations with $%.2f/month potential savings",
		len(collected.recommendations), collected.totalSavings)

	s.persistCollection(ctx, collected.recommendations, collected.successfulCollects, collected.failedProviders)

	// Send notification if we have recommendations
	if len(collected.recommendations) > 0 && collected.totalSavings > 0 {
		// Sort by savings descending to show top recommendations in email
		sort.Slice(collected.recommendations, func(i, j int) bool {
			return collected.recommendations[i].Savings > collected.recommendations[j].Savings
		})
		...
		for i := range collected.recommendations {
			if len(data.Recommendations) >= 10 { // Limit to top 10 in email
				break
			}
			rec := &collected.recommendations[i]
	...
	return &CollectResult{
		Recommendations:     len(collected.recommendations),
		TotalSavings:        collected.totalSavings,
...
// allProvidersResult bundles the five non-error return values of
// collectAllProviders into a single struct so the function does not exceed the
// tooManyResultsChecker threshold.
type allProvidersResult struct {
...
// / recommendations ordering is deterministic regardless of goroutine
...
func (s *Scheduler) collectAllProviders(ctx context.Context, globalCfg *config.GlobalConfig) (allProvidersResult, error) {
	res := allProvidersResult{
		failedProviders: map[string]string{},
	}
	...
	if err := g.Wait(); err != nil {
		return allProvidersResult{}, err
	}
	if cerr := ctx.Err(); cerr != nil {
		return allProvidersResult{}, cerr
	}
	...
	for _, providerName := range globalCfg.EnabledProviders {
		...
		if out.err != nil {
			...
			res.failedProviders[providerName] = out.err.Error()
			continue
		}
		res.successfulProviders = append(res.successfulProviders, providerName)
		res.successfulCollects = append(res.successfulCollects, expandSuccessfulCollects(providerName, out.succeededAccountIDs)...)
		for i := range out.recs {
			res.totalSavings += out.recs[i].Savings
		}
		res.recommendations = append(res.recommendations, out.recs...)
	}
	return res, nil
}
...
func fanOutPerAccount(
	...
	if err := g.Wait(); err != nil { // errs are always nil (swallowed above per-goroutine)
		logging.Warnf("collectAcrossAccounts: unexpected error from errgroup: %v", err)
	}
	return all, outcome
}
...
func (s *Scheduler) fetchAndConvert(ctx context.Context, prov provider.Provider, providerName string, accountID *string, globalCfg *config.GlobalConfig) ([]config.RecommendationRecord, error) {
	...
	if len(recs) == 0 && globalCfg != nil {
		...
		var recsErr error
		recs, recsErr = recClient.GetRecommendations(ctx, params)
		if recsErr != nil {
			logging.Warnf("GetRecommendations (fallback) for %s: %v", providerName, recsErr)
		}
	}
