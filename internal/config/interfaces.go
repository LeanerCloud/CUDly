...
type StoreInterface interface {
	...
	// canceled, setting canceled_by. The 'scheduled' status is intentionally
	// NOT accepted here -- the Gmail-style pre-fire delay revoke path (issue
	// #290) has its own CancelScheduledExecutionAtomic so the two flows surface
	// distinct CAS race outcomes (this path returns 409 on already-approved;
	// scheduled-revoke returns 410 "window closed" on already-fired).
