...
func TestRouter_saveAzureCredentialsHandler(t *testing.T) {
	ctx := context.Background()
	mockAuth := new(MockAuthService)
	adminSession := &Session{UserID: "uid", Role: "admin"}
	mockAuth.On("ValidateSession", ctx, "admin-token").Return(adminSession, nil)

	h := &Handler{auth: mockAuth} // azureCredsARN is empty → error from handler
	r := newTestRouter(h)
	...
	_, err := r.saveAzureCredentialsHandler(ctx, req, nil)
	// Expects error because azureCredsARN is not configured
	assert.Error(t, err)
}

func TestRouter_saveGCPCredentialsHandler(t *testing.T) {
	ctx := context.Background()
	mockAuth := new(MockAuthService)
	adminSession := &Session{UserID: "uid", Role: "admin"}
	mockAuth.On("ValidateSession", ctx, "admin-token").Return(adminSession, nil)

	h := &Handler{auth: mockAuth} // gcpCredsARN is empty → error from handler
	r := newTestRouter(h)
	...
	_, err := r.saveGCPCredentialsHandler(ctx, req, nil)
	assert.Error(t, err)
}
...
// ---------------------------------------------------------------------------
// credentials handlers — saveAzureCredentials / saveGCPCredentials error paths
// ---------------------------------------------------------------------------

func TestHandler_saveAzureCredentials_RequiresAdmin(t *testing.T) {
	h := &Handler{}
	_, err := h.saveAzureCredentials(context.Background(), &events.LambdaFunctionURLRequest{})
	assert.Error(t, err)
}

func TestHandler_saveAzureCredentials_NotConfigured(t *testing.T) {
	ctx := context.Background()
	mockAuth := new(MockAuthService)
	adminSession := &Session{UserID: "uid", Role: "admin"}
	mockAuth.On("ValidateSession", ctx, "admin-token").Return(adminSession, nil)

	h := &Handler{auth: mockAuth, azureCredsARN: ""}
	req := &events.LambdaFunctionURLRequest{
		Headers: map[string]string{"Authorization": "Bearer admin-token"},
		Body:    `{}`,
	}
	_, err := h.saveAzureCredentials(ctx, req)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "not configured")
}

func TestHandler_saveAzureCredentials_InvalidBody(t *testing.T) {
	ctx := context.Background()
	mockAuth := new(MockAuthService)
	adminSession := &Session{UserID: "uid", Role: "admin"}
	mockAuth.On("ValidateSession", ctx, "admin-token").Return(adminSession, nil)

	h := &Handler{auth: mockAuth, azureCredsARN: "arn:aws:secretsmanager:us-east-1:123:secret:azure-creds"}
	req := &events.LambdaFunctionURLRequest{
		Headers: map[string]string{"Authorization": "Bearer admin-token"},
		Body:    `{invalid json}`,
	}
...
func TestHandler_saveAzureCredentials_MissingFields(t *testing.T) {
	ctx := context.Background()
	mockAuth := new(MockAuthService)
	adminSession := &Session{UserID: "uid", Role: "admin"}
	mockAuth.On("ValidateSession", ctx, "admin-token").Return(adminSession, nil)
...
func TestHandler_saveGCPCredentials_RequiresAdmin(t *testing.T) {
	h := &Handler{}
	_, err := h.saveGCPCredentials(context.Background(), &events.LambdaFunctionURLRequest{})
	assert.Error(t, err)
}

func TestHandler_saveGCPCredentials_NotConfigured(t *testing.T) {
	ctx := context.Background()
	mockAuth := new(MockAuthService)
	adminSession := &Session{UserID: "uid", Role: "admin"}
	mockAuth.On("ValidateSession", ctx, "admin-token").Return(adminSession, nil)

	h := &Handler{auth: mockAuth, gcpCredsARN: ""}
	req := &events.LambdaFunctionURLRequest{
		Headers: map[string]string{"Authorization": "Bearer admin-token"},
		Body:    `{}`,
	}
	_, err := h.saveGCPCredentials(ctx, req)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "not configured")
}

func TestHandler_saveGCPCredentials_InvalidType(t *testing.T) {
	ctx := context.Background()
	mockAuth := new(MockAuthService)
	adminSession := &Session{UserID: "uid", Role: "admin"}
	mockAuth.On("ValidateSession", ctx, "admin-token").Return(adminSession, nil)

	h := &Handler{auth: mockAuth, gcpCredsARN: "arn:aws:secretsmanager:us-east-1:123:secret:gcp-creds"}
	req := &events.LambdaFunctionURLRequest{
		Headers: map[string]string{"Authorization": "Bearer admin-token"},
		Body:    `{"type": "user_account", "project_id": "proj", "private_key": "key", "client_email": "e@e.com"}`,
	}
	_, err := h.saveGCPCredentials(ctx, req)
	assert.Error(t, err)
...
// ---------------------------------------------------------------------------
// checkCredentialsStatus / checkAzureCredentials / checkGCPCredentials
// ---------------------------------------------------------------------------

func TestHandler_checkAzureCredentials_NoARN(t *testing.T) {
	h := &Handler{azureCredsARN: ""}
	assert.False(t, h.checkAzureCredentials(context.Background()))
}

func TestHandler_checkGCPCredentials_NoARN(t *testing.T) {
	h := &Handler{gcpCredsARN: ""}
	assert.False(t, h.checkGCPCredentials(context.Background()))
}
