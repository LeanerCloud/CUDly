...
func (r *Router) registerRoutes() {
	r.routes = []Route{
		...
		// (issue #290). Session-authed only; the handler enforces the
		// revoke-any/revoke-own RBAC matrix and the per-provider window check.
...
// revokePurchaseHandler serves both revoke routes that point at the unified
// revokePurchase dispatcher:
//   - POST /api/purchases/{id}/revoke          (AuthUser, in-app/#290): the
//     middleware has already validated the session; resolveApprovalToken
//     returns "" so revokePurchase takes a session path.
//   - GET|POST /api/purchases/revoke/{id}      (AuthPublic, email/#291): the
//     token is carried in the link/form and resolved here.
//
// Rate-limiting the public revoke key is harmless for the AuthUser route.
