...
func buildSuppressions(recs []config.RecommendationRecord, executionID string, cfg *config.GlobalConfig, now time.Time) []config.PurchaseSuppression {
	...
	for i := range recs {
		...
		if recs[i].CloudAccountID != nil {
			accountID = *recs[i].CloudAccountID
		}
		k := key{
			accountID:    accountID,
			provider:     recs[i].Provider,
			service:      recs[i].Service,
			region:       recs[i].Region,
			resourceType: recs[i].ResourceType,
			engine:       recs[i].Engine,
		}
...
func (h *Handler) getPlannedPurchases(ctx context.Context, req *events.LambdaFunctionURLRequest) (*PlannedPurchasesResponse, error) {
	...
	for i := range executions {
		plan := planMap[executions[i].PlanID]
		if plan == nil {
			continue
		}
		ok, err := h.isPlanAllowedCached(ctx, session, executions[i].PlanID, allowedPlan)
		if err != nil {
			return nil, err
		}
		if !ok {
		...
		purchases = append(purchases, buildPlannedPurchase(plan, &executions[i]))
	}
...
func buildPlannedPurchase(plan *config.PurchasePlan, exec *config.PurchaseExecution) PlannedPurchase {
	...
	for k := range plan.Services {
		provider = plan.Services[k].Provider
		service = plan.Services[k].Service
		term = plan.Services[k].Term
		payment = plan.Services[k].Payment
		break
	}
...
func (h *Handler) deletePlannedPurchase(ctx context.Context, req *events.LambdaFunctionURLRequest, executionID string) (*StatusResponse, error) {
	...
	exec, err := h.cancelOrRecoverExecution(ctx, executionID, resolveCreatorUserID(session))
	if err != nil {
		return nil, err
	}
	...
	if exec.PlanID != "" {
		if err := h.disablePlan(ctx, exec.PlanID); err != nil {
			return nil, err
		}
	}
...
func buildScheduledEmailData(dashboardURL string, execution *config.PurchaseExecution, globalCfg *config.GlobalConfig, actor string) email.NotificationData {
	...
	for i := range execution.Recommendations {
		summaries = append(summaries, email.RecommendationSummary{
			Service:      execution.Recommendations[i].Service,
			ResourceType: execution.Recommendations[i].ResourceType,
			Region:       execution.Recommendations[i].Region,
			Count:        execution.Recommendations[i].Count,
			Term:         execution.Recommendations[i].Term,
			Payment:      execution.Recommendations[i].Payment,
			UpfrontCost:  execution.Recommendations[i].UpfrontCost,
		})
	}
...
func (h *Handler) cancelPurchaseViaSession(ctx context.Context, req *events.LambdaFunctionURLRequest, execution *config.PurchaseExecution) (any, error) {
	...
	if err := h.config.WithTx(ctx, func(tx pgx.Tx) error {
		var err error
		cancelOK, currentStatus, err = h.config.CancelExecutionAtomic(ctx, tx, execution.ExecutionID, canceledBy)
		if err != nil {
			return err
		}
		if !cancelOK {
...
func validateAndTotalRecommendations(recs []config.RecommendationRecord) (upfront, savings float64, err error) {
	...
	for i := range recs {
		if recs[i].UpfrontCost < 0 {
			return 0, 0, NewClientError(400, fmt.Sprintf("recommendation %d has negative upfront cost: %.2f", i, recs[i].UpfrontCost))
		}
		if recs[i].Savings < 0 {
			return 0, 0, NewClientError(400, fmt.Sprintf("recommendation %d has negative savings: %.2f", i, recs[i].Savings))
		}
...
func purchaseIdempotencyKey(creatorID string, recs []config.RecommendationRecord, capacityPercent int) string {
	...
	for i := range recs {
		acct := ""
		if recs[i].CloudAccountID != nil {
			acct = *recs[i].CloudAccountID
		}
		tuples = append(tuples, fmt.Sprintf("%s|%s|%s|%s|%s|%s|%d|%d|%s",
			strings.ToLower(strings.TrimSpace(recs[i].Provider)),
			recs[i].Service, recs[i].Region, recs[i].ResourceType, recs[i].Engine, acct,
			recs[i].Count, recs[i].Term, strings.ToLower(strings.TrimSpace(recs[i].Payment))))
	}
	sort.Strings(tuples)
	h := sha256.New()
...
func (h *Handler) executePurchase(ctx context.Context, req *events.LambdaFunctionURLRequest) (any, error) {
	...
	var gracePeriodCfg *config.GlobalConfig
	if g, cfgErr := h.config.GetGlobalConfig(ctx); cfgErr == nil {
		gracePeriodCfg = g
	}
...
func uniqueAccountIDsFromRecs(recs []config.RecommendationRecord) []string {
	...
	var out []string
	for i := range recs {
		if recs[i].CloudAccountID == nil || *recs[i].CloudAccountID == "" {
			continue
		}
