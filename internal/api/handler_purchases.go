...
// revokeCompletedExecution handles revocation of an execution already loaded
// and confirmed to be in a revocable status (completed / partially_completed).
// It is the post-execution-notification arm of the unified revokePurchase
// dispatcher (see handler_purchases_revoke.go): the email one-click link and
// the dashboard Revoke button on a completed row both land here.
//
// It accepts the same three-mode dispatch shape as approvePurchase /
// cancelPurchase:
//
//  1. Session present AND RBAC-authorized (admin / cancel-any /
...
//
// The cloud-side revocation (AWS RI/SP support-case path) is out of scope for
// issue #291; this records a "revocation_requested" status so the History UI
// can surface it. Azure reservation returns (the real cloud-side cancel) go
// through the purchase_history arm in handler_purchases_revoke.go instead.
//
// Present-day behaviour: calling this within the AWS revocation window records
// the cancellation and returns {"status":"revocation_requested"}. Past the
// window it returns a friendly 409/403 with a plain-language message.
func (h *Handler) revokeCompletedExecution(ctx context.Context, req *events.LambdaFunctionURLRequest, execution *config.PurchaseExecution, token string) (any, error) {
