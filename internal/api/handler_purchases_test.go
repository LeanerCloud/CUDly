...
func TestHandler_revokePurchase_PendingExecution(t *testing.T) {
	...
	// auth must be non-nil so the unified fail-closed guard passes; the
	// pending-status 409 short-circuits before any auth method is invoked.
	handler := &Handler{config: mockStore, auth: new(MockAuthService)}
	req := &events.LambdaFunctionURLRequest{
...
func TestHandler_revokePurchase_NotFound(t *testing.T) {
	...
	// auth must be non-nil so the unified fail-closed guard passes; with a token
	// present and no execution, the email-link 404 short-circuits before auth.
	handler := &Handler{config: mockStore, auth: new(MockAuthService)}
	req := &events.LambdaFunctionURLRequest{}
