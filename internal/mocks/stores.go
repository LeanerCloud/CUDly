...
func (m *MockConfigStore) CancelExecutionAtomic(ctx context.Context, tx pgx.Tx, executionID string, cancelledBy *string) (canceled bool, currentStatus string, err error) {
	if !isExpected(&m.Mock, "CancelExecutionAtomic") {
		return true, "cancelled", nil //nolint:misspell // DB schema value 'cancelled' -- see migration 000035_add_execution_attribution.up.sql
	}
...
func (m *MockConfigStore) CancelScheduledExecutionAtomic(ctx context.Context, tx pgx.Tx, executionID string, cancelledBy *string) (canceled bool, currentStatus string, err error) {
...
// isExpected reports whether m has any .On() expectation for method.
