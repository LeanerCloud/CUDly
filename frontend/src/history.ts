...
function statusBadgeHTML(status: string): string {
  ...
  switch (normalized) {
    ...
    case 'canceled':
    case 'cancelled':
      // Migration 000078 (expand-contract rename): the backend may return
...
function renderHistoryList(purchases: HistoryPurchase[]): void {
  ...
  if (activeStatusFilter !== 'all' && !purchases.some(p => {
    ...
    // Migration 000078: the Cancelled chip key is 'cancelled' (British, kept
  ...
  const visible = purchases.filter(p => {
    ...
    if (activeStatusFilter === 'pending') return s === 'pending' || s === 'notified' || isInFlightStatus(s);
    if (activeStatusFilter === 'completed') return s === 'completed' || s === 'partially_completed' || !p.status;
    // Migration 000078: surface BOTH spellings under the Cancelled chip during
