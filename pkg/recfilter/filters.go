...
// PassesDimensions runs the stateless include/exclude checks on region,
// instance type, and engine. Returns false on the first failing filter.
// Account filtering is deliberately out of scope here: it is name-substring
// based and depends on an operator-side account alias cache, so it stays in
// cmd (see cmd/multi_service_filters.go's shouldIncludeAccount).
func (f Filters) PassesDimensions(rec *common.Recommendation) bool {
	if !f.IncludesRegion(rec.Region) {
		return false
	}
	if !f.IncludesInstanceType(rec.ResourceType) {
		return false
	}
	if !f.IncludesEngine(rec) {
		return false
	}
	return true
}
