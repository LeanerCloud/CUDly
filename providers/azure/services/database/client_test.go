...
func TestDatabaseClient_PurchaseCommitment_TagInjection(t *testing.T) {
	...
	rec := common.Recommendation{ResourceType: "GP_Gen5_8", Term: "1yr", Count: 1, CommitmentCost: 5000.0, PaymentOption: "no-upfront"}
...
// TestDatabaseClient_PurchaseCommitment_BillingPlan pins the billingPlan
// wiring (issue #1502, mirroring PR #1495's fix for compute): Azure SQL
// Database reserved capacity supports both Upfront and Monthly billing
// (learn.microsoft.com/azure/azure-sql/database/reservations-discount-overview),
...
func TestDatabaseClient_PurchaseCommitment_BillingPlan(t *testing.T) {
	cases := []struct {
		name          string
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			rec := common.Recommendation{ResourceType: "GP_Gen5_8", Term: "1yr", Count: 1, PaymentOption: tc.paymentOption}
			...
			const orderID = "db-billingplan-test"
			var capturedBody []byte
			mockHTTP.On("Do", mock.MatchedBy(func(r *http.Request) bool {
				if r.URL.Path != "/providers/Microsoft.Capacity/calculatePrice" {
					return false
